...
static void cs35l45_abort_ramp(struct cs35l45_private *cs35l45)
{
	...
	__flush_workqueue(cs35l45->vol_ctl.ramp_wq);
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	ucontrol->value.integer.value[0] = (cs35l45->component_pin_dsp1_state == true) ? 1 : 0;
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ucontrol->value.integer.value[0]) {
		...
		cs35l45->component_pin_dsp1_state = true;
		...
		cs35l45->component_pin_dsp1_state = false;
				 ...
				 struct snd_soc_dai *dai)
{
	...
	dev_dbg(cs35l45->dev, "%s: asp_wl=%d, asp_width=%d, rate=%d, channels=%d\n", __func__,
				asp_wl, asp_width, params_rate(params), params_channels(params));
	
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		dev_dbg(cs35l45->dev, "%s: stream == SNDRV_PCM_STREAM_PLAYBACK\n", __func__);
		regmap_update_bits(cs35l45->regmap, CS35L45_ASP_CONTROL2,
					      ...
					      unsigned int data)
{
	...
	case EVENT_SPEAKER_STATUS:
		...
		dev_dbg(cs35l45->dev, "MBOX 3 event detected (SPEAKER_STATUS = %u)\n", cs35l45->speaker_status);
		break;
...
struct snd_kcontrol *cirrus_snd_soc_card_get_kcontrol(struct snd_soc_card *soc_card, const char *name)
{
				...
				const char *ctl_name, bool active)
{
	...
	kcontrol = cirrus_snd_soc_card_get_kcontrol(component->card, name);
	if (!kcontrol) {
